...
Голосовой цикл: **микрофон → STT (Parakeet 0.6B через sherpa-onnx) → LLM (Gemma 4 через LiteRT-LM) → TTS → сохранение в историю**.
...
6. Сменить модель позже можно на экране **«Настройки»** (иконка шестерёнки в «Истории»): выбор из каталога либо своя ссылка. Смена скачивает новый файл и переинициализирует движок.
