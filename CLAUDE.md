...
- Навигация: `presentation/navigation/AppNavHost.kt`. Нижние вкладки — `roadmap` (Курс) / `vocabulary` / `grammar` / `history`; экраны-детали (`topic/{id}`, `rule/{id}`, `review`, `conversation/...`, `setup`, `settings`) идут поверх, панель на них скрыта. Стартовый экран выбирает `MainActivity` по наличию файла `filesDir/model.litertlm`.
...
- Схема Room — версия 2, миграция `MIGRATION_1_2` в `AppDatabase.kt` (колонка `conversations.topic_id` + таблицы `vocab_progress` и `topic_progress`). `exportSchema = false`, автотеста миграции нет — при изменении сущностей миграцию писать руками и проверять на устройстве с уже установленной сборкой.
